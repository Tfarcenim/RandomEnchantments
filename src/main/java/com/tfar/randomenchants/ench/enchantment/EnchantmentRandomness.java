...
import static com.tfar.randomenchants.init.ModEnchantment.RANDOMNESS;
import static com.tfar.randomenchants.util.GlobalVars.itemList;
import static com.tfar.randomenchants.util.GlobalVars.size;
...
public class EnchantmentRandomness extends Enchantment {
    ...
    public static void removeDefaultDrops(BlockEvent.HarvestDropsEvent e){
        ...
        if (p == null)return;
        World world = p.getEntityWorld();
        if (EnchantmentUtils.stackHasEnch(p.getHeldItemMainhand(),RANDOMNESS) && !world.isRemote) {
            e.getDrops().clear();
        }
    }
}
