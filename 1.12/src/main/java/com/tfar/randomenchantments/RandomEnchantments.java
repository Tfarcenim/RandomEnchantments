...
public class RandomEnchantments {
  ...
  public static boolean checkShooterArrowAndVictim(Entity arrow, Entity victim) {
    if (!(arrow instanceof EntityArrow)) return true;
    Entity shooter = ((EntityArrow) arrow).shootingEntity;
    return !(shooter instanceof EntityPlayer) || !(victim instanceof EntityLivingBase);
  }
  public static boolean isArrowinBlock(Entity arrow, Entity victim){
    if (!(arrow instanceof EntityArrow)) return true;
    Entity shooter = ((EntityArrow) arrow).shootingEntity;
    return !(shooter instanceof EntityPlayer) || victim != null;
  }
}
